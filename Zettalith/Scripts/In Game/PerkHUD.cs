...
namespace Zettalith
{
    class PerkHUD : HUD
    {
        ...
        public PerkHUD(GUI.Collection collection, InGameController igc, PlayerLocal p, ClientSideController csc) : base(igc, p, csc)
        {
            ...
            allPerks = new Perk[]
            {
                ...
                new Perk("Buff cost decrease", "The cost of all \nbuffs is \ndecreased by \n30%", 0.3f, 0.3f, 1, Load.Get<Texture2D>("Buff Shop Button"), 8, InGameController.PlayerIndex - 2, new Mana(6, 6, 6), 0, csc, descriptionCollection),
                new Perk("Ability cost decrease", "The cost of all \nabilities is \ndecreased by 1 \nof each mana \ntype", 0.7f, 0.3f, 1, Load.Get<Texture2D>("Ability Perk"), 9, InGameController.PlayerIndex - 2, new Mana(4, 4, 9), 0, csc, descriptionCollection),
                ...
                new Perk("Essence income 1", "Essence income \nis increased", 0.5f, 0.35f, 1, Load.Get<Texture2D>("Bonus Shop Button"), 11, InGameController.PlayerIndex - 2, new Mana(3, 3, 5), 25, csc, descriptionCollection),
                new Perk("Essence income 2", "Essence income \nis increased \nmore", 0.5f, 0.2f, 1, Load.Get<Texture2D>("Bonus Shop Button"), 11, InGameController.PlayerIndex - 2, new Mana(4, 4, 6), 40, csc, descriptionCollection)
            };

            // Lägg till vilka perks din perk kan gå till och vilka perks som kan gå till den perk.

            allPerks[0].CreateConnections(new Perk[] { allPerks[1], allPerks[2], allPerks[3], allPerks[4] });
            allPerks[4].CreateConnections(new Perk[] { allPerks[5] });
    ...
    class Perk
    {
        ...
        void UsePerk()
        {
            if (Achieved == false && On == true && Achievable == true)
            {
                if (Cost <= InGameController.LocalMana && EssenceCost <= InGameController.LocalEssence)
                {
                ...
                else if (Cost > InGameController.LocalMana || EssenceCost > InGameController.LocalEssence)
                {
                    InGameController.Local.ClientController.Alert("Not enough mana and essence");
                }
                else if (Cost > InGameController.LocalMana)
                {
                    InGameController.Local.ClientController.Alert("Not enough mana");
                }
                else if (EssenceCost > InGameController.LocalEssence)
                {
